...
The public feed does not use email login. Admins upload a CSV where column A is `코드번호` and optional column B is `실명`. Each CSV upload becomes the current valid code list: new code numbers are inserted, existing matching members keep their passwords unchanged, and code numbers missing from the CSV are removed from the valid code list. Existing member login records and passwords are preserved, so uploading the same code number again can restore access with the member's existing password. General users can sign up only when their 코드번호 matches that uploaded list, then log in with 코드번호 and their password. Signup and password reset require a password of at least 8 characters with confirmation. Members can reset their password from the login screen when their 실명 and 코드번호 match. Comments use the verified member name stored at signup. Admins can pin posts globally with the upload form after the latest `supabase-schema.sql` has been run.

Security hardening notes: signup and password reset now require passwords of at least 8 characters. Repeated failed member logins are rate-limited after 5 failures within 10 minutes. Admin feed access tokens expire after 3 hours. Login, signup, password reset, download, share, post create, post update, and post delete events are written to `fp_activity_logs` after the latest `supabase-schema.sql` has been run.
