...
The public feed does not use email login. Admins upload a CSV where column A is `소속지점` and column B is `코드번호`. General users can sign up only when their 소속지점 and 코드번호 match that uploaded list, then log in with 코드번호 and their password.
